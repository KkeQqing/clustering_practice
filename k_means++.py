...
# ==================== 函数：（肘部法则）自动计算最佳 K 的函数 ====================
def get_best_k(points, max_k=10):
    sse = []
    ...
    for k in K_range:
        ...
        sse_k = 0
        for i in range(k):
            sse_k += np.sum((points[labels==i] - centers[i])**2)
        sse.append(sse_k)

    # 2. 计算二阶导数 → 找拐点（自动肘部法则）
    sse = np.array(sse)
    d2 = np.diff(np.diff(sse))  # 二阶差分找拐点
    best_idx = np.argmax(np.abs(d2))# 曲率最大的位置 = 拐点
    return K_range[best_idx + 1]

#聚类
# 自动获取最佳 K
best_k = get_best_k(points, max_k=10)
print("✅ 自动计算最佳聚类数量 K =", best_k)

# 使用最佳K聚类
labels, final_centers = kmeans(points, k=best_k)
